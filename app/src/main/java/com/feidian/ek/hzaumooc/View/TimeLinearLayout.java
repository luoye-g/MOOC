...
public class TimeLinearLayout extends LinearLayout {
    ...
    public TimeLinearLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        lineColor = attr.getColor(R.styleable.UnderLineLinearLayout_line_color, 0xff000000);//0xff3dd1a5);
    ...
    private void drawFirstChildViewVertical(Canvas canvas) {
        if (getChildAt(0) != null) {//第一个子视图不为空
            ...
            //画一个圆
            canvas.drawCircle(firstX, firstY+40, pointSize, pointPaint);//画第一个圈
        }
    }
    ...
    private void drawLastChildViewVertical(Canvas canvas) {
        if (getChildAt(getChildCount() - 1) != null) {
            ...
            lastY = top + getChildAt(getChildCount() - 1).getPaddingTop() + lineDynamicDimen;*/
            //画一个图
            canvas.drawCircle(lastX, lastY+40, pointSize, pointPaint);//画第一个圈
    ...
    private void drawBetweenLineVertical(Canvas canvas) {
        for (int i = 0; i < getChildCount() - 1; i++) {
            //画剩下的
            canvas.drawLine(lineMarginSide, firstY+45, lineMarginSide, lastY+40, linePaint);
            //画了线，就画圆
            if (getChildAt(i) != null && i != 0) {
                int top = getChildAt(i).getTop();
                //记录值
                int Y = top + getChildAt(i).getPaddingTop() + lineDynamicDimen;
                canvas.drawCircle(lineMarginSide, Y+45, pointSize, pointPaint);
            }
        }
    }
}
